...
class work_bulk_edit(delegate.page):
    ...
    def POST(self):
        ...
        for subject in shared_tags:
            ...
            changed_tags[subject]['remove'] = [t for t in shared_tags[subject] if t not in form_subject]
            changed_tags[subject]['add'] = [t for t in form_subject if t not in shared_tags[subject]]
        ...
        for work in works:
            ...
            for subject in changed_tags:
                if changed_tags[subject]['remove'] or changed_tags[subject]['add']:
                    ...
                    changeset[subject] = [tag for tag in changeset[subject] if tag not in changed_tags[subject]['remove']]
        ...
        saveutil.commit(comment=comment, action="edit-book")
        return render_template(
            "message.html", "Test", json.dumps(output)
        )
